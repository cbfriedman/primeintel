// Run with: npx tsx workers/text-extraction/run-extract-text.ts
// Limit batch size: npx tsx workers/text-extraction/run-extract-text.ts --limit 3
import { loadEnvLocal } from '../scraper/env';
import { runTextExtraction } from './save-extracted-text';
...
async function main() {
  ...
  console.log('Running PrimeIntel PDF text extraction pipeline...\n');
  console.log(`Processing limit: ${limit} pending document(s)\n`);

  try {
    const result = await runTextExtraction({ limit });
    ...
    console.log(`Documents OCR required: ${result.documents_ocr_required}`);
