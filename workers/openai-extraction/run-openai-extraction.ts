// Run with: npx tsx workers/openai-extraction/run-openai-extraction.ts
// Limit batch size: npx tsx workers/openai-extraction/run-openai-extraction.ts --limit 1
import { loadEnvLocal } from '../scraper/env';
import { runOpenAIExtraction } from './save-openai-extraction';
...
async function main() {
  ...
  console.log('Running PrimeIntel OpenAI AI extraction pipeline...\n');
  console.log(`Processing limit: ${limit} eligible document(s)\n`);

  try {
    const result = await runOpenAIExtraction({ limit });
