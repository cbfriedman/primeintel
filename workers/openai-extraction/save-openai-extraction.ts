...
const LOG_PREFIX = '[save-openai-extraction]';
...
export type OpenAIExtractionRunResult = {
...
export type RunOpenAIExtractionOptions = {
  limit?: number;
};
...
export async function loadEligibleDocuments(
  limit: number,
  model: string,
...
export async function runOpenAIExtraction(
  options: RunOpenAIExtractionOptions = {},
): Promise<OpenAIExtractionRunResult> {
  ...
  const errors: string[] = [];
  const extractedDocs: OpenAIExtractionRunResult['extractedDocs'] = [];
  ...
  for (const doc of documents) {
    ...
    try {
      ...
      const { output, rawResponse, usage } = await extractWithOpenAI(extractionInput);
